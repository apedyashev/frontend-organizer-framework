...
class Rrs.Observer
  ...
  emit: (inNamespace, inSignalName, inData)->
    ...
    if _listeners[namespacedSignal]?
      for listener in _listeners[namespacedSignal]
        Rrs.logger.debug "Emitting namespaced signal #{namespacedSignal}.", listener
        listener.callback?.call(listener.context, inData)
    ...
    if _listeners[inSignalName]?
      for listener in _listeners[inSignalName]
  ...
  listen: (inNamespace, inSignalName, callback, context)->
    ...
    _listeners[signal] = [] unless _listeners[signal]?

    _listeners[signal].push  
      callback  : callback
      context   : context
